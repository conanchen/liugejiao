...
class Vanke0Strategy:
    """万科A迭代0策略：基准·纯持有万科A（无任何策略）
    ...
    def backtest(self):
        ...
        # 考虑交易成本
        initial_investment = self.initial_capital * (1 - self.transaction_cost)
        self.portfolio['cash'] = self.initial_capital - initial_investment
        
        # 计算持仓价值
        if not self.data.empty and 'close' in self.data.columns and len(self.data['close']) > 0:
            self.portfolio['holdings'] = initial_investment * (self.data['close'] / self.data['close'].iloc[0])
...
if __name__ == "__main__":
    # 创建策略实例
    strategy = Vanke0Strategy(start_date="2014-10-01", end_date="2024-09-30")
