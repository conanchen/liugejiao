...
class Vanke3Strategy:
    """万科A迭代3策略：基础框架·估值+周线趋势+震荡期优化（解决"震荡期收益低"）
    保留迭代2的估值+周线趋势框架，增加震荡期识别机制和高抛低吸策略，提升震荡期收益
    """
    ...
    def get_data(self):
        ...
        try:
            ...
            # 打印最近10天的数据
            print("\n=== 最近10天的日线数据 ===")
            print(self.data.tail(10)[['close', 'ma5', 'ma10', 'ma20', 'volatility', 'pe_percentile']].to_string())
    ...
    def generate_signals(self):
        ...
        # 应用震荡期调整
        self.signals['final_position_before_oscillation'] = self.signals['adjusted_position']
        self.signals['final_position'] = self.signals['adjusted_position'] + self.signals['oscillation_adj']
        
        # 确保现金仓不低于10%，股票仓位不超过90%
        self.signals['final_position'] = np.minimum(self.signals['final_position'], 1 - self.cash_floor)
        self.signals['final_position'] = np.maximum(self.signals['final_position'], self.cash_floor)
        
        # 为了平滑仓位变化，使用移动平均
        self.signals['final_position'] = self.signals['final_position'].rolling(window=10).mean().fillna(0.5)
    ...
    def backtest(self):
        """回测策略：基于PE分位数、周线趋势和震荡期优化的仓位调整策略"""
    ...
    def analyze(self):
        ...
        try:
            ...
            if oscillation_periods.sum() > 0:
                ...
                print(f"震荡期表现: 累计收益率 {oscillation_cum_return:.2f}%")
            
            # 打印结果
            print(f"迭代3策略表现（{self.start_date}至{self.end_date}）：")
            ...
            # 绘制仓位变化曲线
            plt.figure(figsize=(12, 6))
            plt.plot(self.signals['final_position_before_oscillation'] * 100, label='优化前仓位（%）')
            plt.plot(self.signals['final_position'] * 100, label='优化后仓位（%）')
            plt.title('万科A迭代3策略仓位变化')
            plt.xlabel('日期')
            plt.ylabel('仓位百分比（%）')
            plt.legend()
            plt.grid(True)
            ...
            plt.fill_between(self.signals.index, 0, 100, where=self.signals['is_oscillating'], alpha=0.2, color='gray', label='震荡期')
            plt.plot(self.data['close'], 'blue', label='万科A价格')
            plt.title('万科A迭代3策略震荡期识别')
            plt.xlabel('日期')
            plt.ylabel('价格（元）')
            plt.legend()
            plt.grid(True)
...
if __name__ == "__main__":
    # 创建策略实例
    strategy = Vanke3Strategy(start_date="2014-10-01", end_date="2024-09-30")
